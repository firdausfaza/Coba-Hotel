...
// FORM TAMBAH ROOM #INSERT
	...
	// panggil data
      $nama = htmlspecialchars($data["namacust"]);
      $nmkamar = htmlspecialchars($data["room"]);
      $class = htmlspecialchars($data["nokamar"]);
      $cekin = htmlspecialchars($data["cekin"]);
      $cekout = htmlspecialchars($data["cekout"]);
      $bed = htmlspecialchars($data["ttlharga"]);

      // insert data
      $query = "INSERT INTO dt_verify VALUES ('', '$nama', '$nmkamar', '$class', '$cekin', '$cekout', '$bed')";
	...
	function verify($data){
		...
		$query =  "UPDATE dt_transaksi SET verify = 'Sudah Checkin'";
	...
	function hapusvisit($id){

		global $koneksi;

		mysqli_query($koneksi, "DELETE FROM dt_verify WHERE id = $id");
	...
	function test($id){
		...
		mysqli_query($koneksi,"SELECT id FROM dt_room");

		return mysqli_num_rows($koneksi);
